...
export function AdminLocationsPage() {
  ...
  const columns: TableColumn<Location>[] = [
    { key: 'name', header: 'Name', render: (location) => <span className="font-medium text-slate-950">{location.name}</span> },
    { key: 'code', header: 'Code', render: (location) => location.code },
    { key: 'status', header: 'Status', render: (location) => <Badge tone={location.status === 'ACTIVE' ? 'success' : 'neutral'}>{location.status}</Badge> },
    {
      key: 'detail',
      header: '',
      render: (location) => (
  ...
  return (
    <SectionContainer>
      ...
      />
      <div className="mt-5">
        <Table items={locations} columns={columns} getRowKey={(location) => location.id} emptyTitle="No locations" />
      </div>
    </SectionContainer>
  );
}
