import { BookOpen, CalendarRange, LayoutDashboard, LogOut, Menu, MapPinned } from 'lucide-react';
...
function AdminNav() {
  return (
    <nav className="space-y-1">
      {nav.map((item) => (
        <NavLink key={item.to} to={item.to} className={({ isActive }) => cn('flex h-10 items-center gap-2 rounded-md px-3 text-sm font-medium', isActive ? 'bg-teal-50 text-teal-800' : 'text-slate-600 hover:bg-slate-100')}>
...
export function AdminLayout() {
  ...
  return (
    <div className="min-h-svh bg-slate-50 lg:grid lg:grid-cols-[260px_1fr]">
      ...
      <Drawer open={adminDrawerOpen} title="Admin Console" onClose={closeAdminDrawer}>
        <AdminNav />
      </Drawer>
      <div>
        <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/95 backdrop-blur">
          <div className="flex h-16 items-center justify-between px-4 sm:px-6 lg:px-8">
            <div className="flex items-center gap-3">
              <Button type="button" variant="ghost" className="h-10 w-10 px-0 lg:hidden" onClick={openAdminDrawer} aria-label="Open admin navigation">
            ...
            <Button type="button" variant="ghost" onClick={logout} icon={<LogOut className="h-4 w-4" />}>Logout</Button>
          </div>
        </header>
        <Outlet />
      </div>
