...
export function LockerVisualizationGrid({
  lockers,
  selectedLockerId,
  onSelect,
}: {
  ...
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
      {lockers.map((locker) => {
        const available =
          locker.visualizationStatus === "AVAILABLE" &&
          locker.lockerStatus === "AVAILABLE";
        const selected = locker.id === selectedLockerId;
        const Icon =
          locker.visualizationStatus === "MY_BOOKING"
            ? UserCheck
            : available
              ? CheckCircle2
              : Lock;
        return (
          <button
            ...
            className={cn(
              "h-fit min-h-28 rounded-lg border bg-white p-3 text-left transition disabled:opacity-70",
              selected
                ? "border-teal-700 ring-2 ring-teal-700/10"
                : "border-slate-200",
              available ? "hover:border-teal-300" : "",
            )}
          >
            <div className="flex items-start justify-between gap-2">
              ...
              <Icon
                className={cn(
                  "h-5 w-5",
                  available ? "text-teal-700" : "text-slate-400",
                )}
              />
            </div>
            <Badge
